...
import net.optifine.util.PhotonSM;
...
public class SVertexBuilder {
   ...
   public static void initVertexBuilder(BufferBuilder wrr) {
      ((ExtendedBufferBuilder) wrr).setVertexBuilder(new SVertexBuilder());
   }
   ...
   public static void pushEntity(BlockState blockState, BlockPos blockPos, BlockView blockAccess, BufferBuilder wrr) {
      Block block = blockState.getBlock();
      int i;
      int j;


      i = Block.getIdByBlock(block);
      j = block.getMeta(blockState);
      ...
      int k = block.getBlockType();
      int l = ((k & 65535) << 16) + (i & 65535);
      int i1 = j & 65535;
      ((ExtendedBufferBuilder) wrr).getVertexBuilder().pushEntity(((long) i1 << 32) + (long) l);
   }

   public static void popEntity(BufferBuilder wrr) {
      ((ExtendedBufferBuilder) wrr).getVertexBuilder().popEntity();
   }
   ...
   public static void endSetVertexFormat(BufferBuilder wrr) {
      SVertexBuilder svertexbuilder = ((ExtendedBufferBuilder) wrr).getVertexBuilder();
      VertexFormat vertexformat = wrr.getFormat();
      ...
      svertexbuilder.offsetNormal = svertexbuilder.hasNormal ? vertexformat.getNormalIndex() / 4 : 0;
      svertexbuilder.offsetUV = svertexbuilder.hasUV ? vertexformat.getUvIndex(0) / 4 : 0;
   ...
   public static void endAddVertex(BufferBuilder wrr) {
      ...
      if (svertexbuilder.vertexSize == 14) {
         if (wrr.getDrawMode() == 7 && wrr.getVertexCount() % 4 == 0) {
            svertexbuilder.calcNormal(wrr, wrr.getVertexCount() * wrr.getFormat().getVertexSizeInteger() - 4 * svertexbuilder.vertexSize);
         }

         long i = svertexbuilder.entityData[svertexbuilder.entityDataIndex];
         int j = wrr.getVertexCount() * wrr.getFormat().getVertexSizeInteger() - 14 + 12;
         wrr.intBuffer.put(j, (int) i);
         wrr.intBuffer.put(j + 1, (int) (i >> 32));
      }
   }
   ...
   public static void endAddVertexData(BufferBuilder wrr) {
      ...
      if (svertexbuilder.vertexSize == 14 && wrr.getDrawMode() == 7 && wrr.getVertexCount() % 4 == 0) {
         svertexbuilder.calcNormal(wrr, wrr.getVertexCount() * wrr.getFormat().getVertexSizeInteger() - 4 * svertexbuilder.vertexSize);
      }
   }

   public void calcNormal(BufferBuilder wrr, int baseIndex) {
      FloatBuffer floatbuffer = wrr.getByteBuffer().asFloatBuffer();
      IntBuffer intbuffer = wrr.intBuffer;
      int i = wrr.getVertexCount() * wrr.getFormat().getVertexSizeInteger();
   ...
   public static void calcNormalChunkLayer(BufferBuilder wrr) {
      if (wrr.getFormat().hasNormal() && wrr.getDrawMode() == 7 && wrr.getVertexCount() % 4 == 0) {
         SVertexBuilder svertexbuilder = ((ExtendedBufferBuilder) wrr).getVertexBuilder();
         endSetVertexFormat(wrr);
   ...
   public static void drawArrays(int drawMode, int first, int count, BufferBuilder wrr) {
      if (count != 0) {
         ...
         if (i == 56) {
            ...
            PhotonSM.glDrawArrays(drawMode, first, count);
