         ...
         if (this.indexNames2 == null || index2 >= 0 && index2 <= this.indexNames2.length) {
            switch (this) {
               case BIOME: {
                  ...
                  return (float)biome.id;
               }
               case TEMPERATURE:
                  BlockPos var5 = Shaders.getCameraPosition();
                  Biome var7 = Shaders.getCurrentWorld().getBiome(var5);
                  return var7 != null ? var7.getTemperature(var5) : 0.0F;
               case RAINFALL: {
                  BlockPos pos = Shaders.getCameraPosition();
                  Biome biome = Shaders.getCurrentWorld().getBiome(pos);
            ...
            Photon.LOGGER.warn("Invalid index2, parameter: " + this + ", index: " + index2);
            return 0.0F;
         }
      } else {
         Photon.LOGGER.warn("Invalid index1, parameter: " + this + ", index: " + index1);
         return 0.0F;
      }
   }
